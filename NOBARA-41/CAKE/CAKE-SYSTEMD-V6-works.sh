...
[Service]
Type=oneshot
ExecStart=/bin/bash -c 'interface=\$(ip link show | awk -F: '\''\$0 ~ \"wlp|wlo|wlx\" && \$0 !~ \"NO-CARRIER\" {gsub(/^[ \t]+|[ \t]+$/, \"\", \$2); print \$2; exit}'\''); if [ -n \"\$interface\" ]; then sudo tc qdisc replace dev \$interface root cake bandwidth 1Gbit diffserv3 triple-isolate nonat nowash no-ack-filter split-gso rtt 50ms raw overhead 0; fi'
RemainAfterExit=yes
