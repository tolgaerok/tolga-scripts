...
while true; do
  ...
  for folder in "${INCLUDE_FOLDERS[@]}"; do
      ...
      if [[ "$folder" == "Desktop" ]]; then
          rsync -avz --progress --exclude="${EXCLUDE_DIRS[@]}" --update --stats --exclude="*.iso*" "$SOURCE_DIR/$folder/" "$DEST_DIR/$folder/"
      else
          rsync -avz --progress --exclude="${EXCLUDE_DIRS[@]}" --update --stats "$SOURCE_DIR/$folder/" "$DEST_DIR/$folder/"
      fi
